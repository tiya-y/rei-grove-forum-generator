...
export async function generatePosts(params: {
  ...
  const message = await client.messages.create({
    ...
    max_tokens: 4096,
    system: `You write short forum discussion-starter posts for the REI Grove community forums. Goal: spark real replies from other members.\n\nSTRICT FORMAT RULES:\n- Body is exactly 2-3 short sentences. Brief and scannable, like something a busy person can read in five seconds.\n- Never use an em dash (—) anywhere in the title or body. Use a period, comma, or separate sentence instead.\n- No corporate or moderator phrasing: no "our community," "we'd love to hear," "here at REI Grove," "as always."\n- No engagement-bait tack-on lines: no "drop your number below," "comment your thoughts," "tell me below," etc. The question itself is the whole post.\n\n${BRAND_VOICE}\n\nReal threads members have posted (for tone and format calibration only, don't copy or lightly reword these):\n${REAL_THREAD_EXAMPLES}\n\nREI Grove content library:\n${RESOURCE_LIBRARY}`,
    messages: [
      {
        role: "user",
        content: `Generate ${count} forum posts for the "${category}" category.\n\n${sourceInstruction}\n\n${postTypeInstruction}\n\nEach post should invite replies: ask a genuine question, invite people to share their own experience, or start a debate. Vary how experienced the poster sounds across the batch, the way the real threads above do: roughly a third can be specific and numbers-driven, but most should read like an average landlord, not an underwriter, plain language, no jargon assumed. Where a batch includes more than one post, make at least one of them a humble, open-ended question from someone who sounds like they're still learning, and at least one where the poster is straight up asking what a term or acronym means or how it actually works, instead of using it correctly in a sentence. Keep titles punchy. Call propose_posts with the full batch.`,
      },
    ],
